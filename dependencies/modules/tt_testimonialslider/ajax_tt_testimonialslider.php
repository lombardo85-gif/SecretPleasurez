...
include_once('tt_testimonialslider.php');

$home_slider = new Tt_Testimonialslider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'testislider_slides` SET `position` = '.(int)$position.'
			WHERE `id_testislider_slides` = '.(int)$id_slide
		);
