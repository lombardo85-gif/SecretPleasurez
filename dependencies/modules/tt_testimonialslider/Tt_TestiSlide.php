...
class Tt_TestiSlide extends ObjectModel
{
	public $title;
	public $description;
	//public $url;
	...
	public static $definition = array(
		'table' => 'testislider_slides',
		'primary' => 'id_testislider_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'testislider` (`id_shop`, `id_testislider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'testislider`
			WHERE `id_testislider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_testislider_slides` = hs.`id_testislider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_testislider_slides` as id_slide
			FROM `'._DB_PREFIX_.'testislider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'testislider` hs ON (hss.`id_testislider_slides` = hs.`id_testislider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new Tt_TestiSlide($row['id_slide']);
