...
class Tt_ShipGrid extends ObjectModel
{
	public $title;
	public $description;
	//public $url;
	...
	public static $definition = array(
		'table' => 'shipgrid_slides',
		'primary' => 'id_shipgrid_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'shipgrid` (`id_shop`, `id_shipgrid_slides`)
	...
	public function delete()
	{
		...
		{
			if (preg_match('/testimonial_img/', $image) === 0)
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'shipgrid`
			WHERE `id_shipgrid_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_shipgrid_slides` = hs.`id_shipgrid_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_shipgrid_slides` as id_slide
			FROM `'._DB_PREFIX_.'shipgrid_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'shipgrid` hs ON (hss.`id_shipgrid_slides` = hs.`id_shipgrid_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new Tt_ShipGrid($row['id_slide']);
