...
include_once('tt_shippinggrid.php');

$home_slider = new Tt_Shippinggrid();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'shipgrid_slides` SET `position` = '.(int)$position.'
			WHERE `id_shipgrid_slides` = '.(int)$id_slide
		);
