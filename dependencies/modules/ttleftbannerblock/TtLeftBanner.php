...
class TtLeftBanner extends ObjectModel
{
	public $title;
	public $url;
	public $image;
	...
	public static $definition = array(
		'table' => 'ttleftbannerblock_slides',
		'primary' => 'id_ttleftbannerblock_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'ttleftbannerblock` (`id_shop`, `id_ttleftbannerblock_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'ttleftbannerblock`
			WHERE `id_ttleftbannerblock_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_ttleftbannerblock_slides` = hs.`id_ttleftbannerblock_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_ttleftbannerblock_slides` as id_slide
			FROM `'._DB_PREFIX_.'ttleftbannerblock_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'ttleftbannerblock` hs ON (hss.`id_ttleftbannerblock_slides` = hs.`id_ttleftbannerblock_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new TtLeftBanner($row['id_slide']);
