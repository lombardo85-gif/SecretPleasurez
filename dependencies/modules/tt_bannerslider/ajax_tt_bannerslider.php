...
include_once('tt_bannerslider.php');

$home_slider = new Tt_Bannerslider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'banslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_banslider_slides` = '.(int)$id_slide
		);
