...
class TtSubBan extends ObjectModel
{
	public $title;
	public $url;
	public $image;
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'ttsubbanner` (`id_shop`, `id_homeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'ttsubbanner`
	...
	public function reOrderPositions()
	{
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM `'._DB_PREFIX_.'ttsubbanner_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'ttsubbanner` hs ON (hss.`id_homeslider_slides` = hs.`id_homeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new TtSubBan($row['id_slide']);
