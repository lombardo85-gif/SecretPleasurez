...
include_once('Ttslideshows.php');

$home_slider = new Ttslideshows();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'ttslideshows_slides` SET `position` = '.(int)$position.'
			WHERE `id_ttslideshow` = '.(int)$id_slide
		);
