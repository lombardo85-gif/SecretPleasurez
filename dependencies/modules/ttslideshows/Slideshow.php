...
class Slideshow extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'ttslideshows_slides',
		'primary' => 'id_ttslideshow',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'ttslideshows` (`id_shop`, `id_ttslideshow`)
	...
	public function delete()
	{
		...
		foreach ($images as $image)
		{
			if (preg_match('/sample/', $image) === 0)
				if ($image && file_exists(dirname(__FILE__).'views/images/'.$image))
					$res &= @unlink(dirname(__FILE__).'views/images/'.$image);
		}

		$res &= $this->reOrderPositions();

		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'ttslideshows`
			WHERE `id_ttslideshow` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_ttslideshow` = hs.`id_ttslideshow` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_ttslideshow` as id_slide
			FROM `'._DB_PREFIX_.'ttslideshows_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'ttslideshows` hs ON (hss.`id_ttslideshow` = hs.`id_ttslideshow`)
